setwd("/Users/emmeke/surfdrive/Longitudinal behavior data Bill/data")
...
for(i in 1:10){
  setwd("/Users/emmeke/surfdrive/Longitudinal behavior data Bill/data/Cluster data therapist")
  # setwd("C:/Users/0446564/surfdrive/Longitudinal behavior data Bill/data/Cluster data therapist")
  # workdata.ther <- read.csv2(input1[i], header = TRUE, dec = ".", sep = ";")[,-1]
  ...
  workdata.ther$T_Vocalizing <- factor(workdata.ther$T_Vocalizing, levels = c("Voc_speak", "Voc_bch",  "Voc_no"))
  workdata.ther$T_Vocalizing <- unclass(workdata.ther$T_Vocalizing)
  workdata.ther$T_Looking <- factor(workdata.ther$T_Looking, levels = c("Look_look", "Look_no"))
  workdata.ther$T_Looking <- unclass(workdata.ther$T_Looking)
  # workdata.ther$T_Head <- factor(workdata.ther$T_Head, levels = c("Head_nod", "Head_shake", "Head_other", "Head_no"))
  # workdata.ther$T_Head <- unclass(workdata.ther$T_Head)
  # workdata.ther$T_Leg <- factor(workdata.ther$T_Leg, levels = c( "Leg_move", "Leg_no"))
  # workdata.ther$T_Leg <- unclass(workdata.ther$T_Leg)
  # workdata.ther$T_Hand_L <- factor(workdata.ther$T_Hand_L, levels = c("Hand_L.gest", "Hand_L.botou", "Hand_L.obtou", "Hand_L.other", "Hand_L.no"))
  # workdata.ther$T_Hand_L <- unclass(workdata.ther$T_Hand_L)
  # workdata.ther$T_Hand_R <- factor(workdata.ther$T_Hand_R, levels = c("Hand_R.gest", "Hand_R.botou", "Hand_R.obtou", "Hand_R.other", "Hand_R.no"))
  # workdata.ther$T_Hand_R <- unclass(workdata.ther$T_Hand_R)

  setwd("/Users/emmeke/surfdrive/Longitudinal behavior data Bill/data/Cluster data patient")
  # setwd("C:/Users/0446564/surfdrive/Longitudinal behavior data Bill/data/Cluster data patient")
  # workdata.pat	 <- read.csv2(input2[i], header = TRUE, dec = ".", sep = ";")[,-1]
  workdata.pat <- read.csv2(input2[i], header = TRUE, dec = ".", sep = ";")[,2:3]
  colnames(workdata.pat) <- paste("P", colnames(workdata.pat), sep = "_")
  # make sure factors levels are in correct order
  workdata.pat$P_Vocalizing <- factor(workdata.pat$P_Vocalizing, levels = c("Voc_speak", "Voc_bch",  "Voc_no"))
  workdata.pat$P_Vocalizing <- unclass(workdata.pat$P_Vocalizing)
  workdata.pat$P_Looking <- factor(workdata.pat$P_Looking, levels = c("Look_look", "Look_no"))
  workdata.pat$P_Looking <- unclass(workdata.pat$P_Looking)
  # workdata.pat$P_Head <- factor(workdata.pat$P_Head, levels = c("Head_nod", "Head_shake", "Head_other", "Head_no"))
  # workdata.pat$P_Head <- unclass(workdata.pat$P_Head)
  # workdata.pat$P_Leg <- factor(workdata.pat$P_Leg, levels = c( "Leg_move", "Leg_no"))
  # workdata.pat$P_Leg <- unclass(workdata.pat$P_Leg)
  # workdata.pat$P_Hand_L <- factor(workdata.pat$P_Hand_L, levels = c("Hand_L.gest", "Hand_L.botou", "Hand_L.obtou", "Hand_L.other", "Hand_L.no"))
  # workdata.pat$P_Hand_L <- unclass(workdata.pat$P_Hand_L)
  # workdata.pat$P_Hand_R <- factor(workdata.pat$P_Hand_R, levels = c("Hand_R.gest", "Hand_R.botou", "Hand_R.obtou", "Hand_R.other", "Hand_R.no"))
