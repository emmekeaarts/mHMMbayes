...
test_that("print mHMM", {
  ...
  expect_output(print(out_2st_simb), paste("AIC over all subjects:", 370))
...
test_that("output PD_subj", {
  ...
  # calculations
  expect_equal(as.numeric(out_2st_simb$PD_subj[[2]][10,(m*q_emiss[1] + 2)]), 0.2212, tolerance=1e-4)
  expect_equal(as.numeric(out_2st_simb$PD_subj[[2]][10,( m*q_emiss[1] + m*q_emiss[2] + m * m + 1)]), -176.7565, tolerance=1e-4)
})
...
test_that("output emis_cov_bar", {
  ...
  # test calcultations
  expect_equal(as.numeric(out_2st_sim_cov1$emiss_cov_bar[[1]][11,  m * (q_emiss[1] - 1)]), -0.0420201, tolerance=1e-4)
  expect_equal(as.numeric(out_2st_sim_cov1$emiss_cov_bar[[2]][11,  m * (q_emiss[2] - 1)]), -0.4754399, tolerance=1e-4)
})
...
test_that("output emiss_int_bar", {
  ...
  # calculations
  expect_equal(as.numeric(out_2st_simb$emiss_int_bar[[1]][10,m * (q_emiss[1] - 1)]), 2.01028, tolerance=1e-4)
  expect_equal(as.numeric(out_2st_simb$emiss_int_bar[[2]][11,m * (q_emiss[2] - 1) - 1]), -2.327577, tolerance=1e-4)
})
...
test_that("output emiss_naccept", {
  ...
  # calculations
  expect_equal(out_2st_simb$emiss_naccept[[1]][9,2], 0)
...
test_that("output emiss_prob_bar", {
  ...
  # calculations
  expect_equal(as.numeric(out_2st_simb$emiss_prob_bar[[1]][11, q_emiss[1]]),  0.05303505, tolerance = 1e-4)
  expect_equal(as.numeric(out_2st_sim_cov1$emiss_prob_bar[[2]][10, q_emiss[2]]),0.4328649, tolerance = 1e-4)
})
...
test_that("output gamma_cov_bar", {
  ...
  # calculations
  expect_equal(as.numeric(out_2st_sim_cov2$gamma_cov_bar[10, m * (m-1) - 1]), 0.2858675, tolerance = 1e-4)
})
...
test_that("output gamma_int_bar", {
  ...
  # calculations
  expect_equal(as.numeric(out_2st_sim_cov2$gamma_int_bar[11, m - 1]), -1.354093, tolerance = 1e-4)
})
...
test_that("output gamma_int_subj", {
  ...
  # calculations
  expect_equal(as.numeric(out_2st_simb$gamma_int_subj[[1]][11, 2 * (m-1)]), -0.4696491, tolerance = 1e-4)
})
...
test_that("output gamma_naccept", {
  ...
  # calculations
  expect_equal(out_2st_simb$gamma_naccept[8,3], 4)
})
...
test_that("output gamma_prob_bar", {
  ...
  # calculations
  expect_equal(as.numeric(out_2st_simb$gamma_prob_bar[11,1]),0.7447745, tolerance = 1e-4)
  expect_equal(as.numeric(out_2st_simb$gamma_prob_bar[10,8]), 0.2787329, tolerance = 1e-4)
})
