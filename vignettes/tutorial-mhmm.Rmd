 ...
 * `mu0`, a matrix with one row (when not including covariates in the model) and the number of columns equal to the number of intercepts to be estimated, containing the hypothesized mean values of the intercepts.
...
set.seed(9843)
out_2st_b <- mHMM_mnl(s_data = nonverbal, 
                      gen = list(m = m, n_dep = n_dep, q_emiss = q_emiss), 
                      start_val = c(list(start_TM), start_EM),
                      mcmc = list(J = 1000, burn_in = 200))
