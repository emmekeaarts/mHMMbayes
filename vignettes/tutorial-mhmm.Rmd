...
Lastly, user can also plot the posterior emission probability distribution related to each state of a model. As an input we use objects coming from `obtain_emiss` function:
```{r emission distributions objects}
# Group level emission probability distribution list:
...
We can display distributions grouped by dependent variables showing how for each `n_dep` dependent variable its probabilities are distributed for each  `m` states, or grouped by states, showing how for each of `m` states the `n_dep` dependent variables are distributed:

```{r show barplots of emission probability distribution}

# Emission probability distribution at the group level displayed by states and for subject number 2 displayed by dependent variable  

group_labs<-c("voc_patient","look_patient","voc_therapist","look_therapist")


plot.mHMM_emiss(emiss=emiss_pop,q_emiss =q_emiss,m=m,n_dep =n_dep, group_labs = group_labs)

plot.mHMM_emiss(emiss=emiss_subj,q_emiss =q_emiss,m=m,n_dep =n_dep,group_labs = group_labs,subj_nr = 2,mode=2)

```
Here, `emiss` is an object of `obtain_emiss` class, `q_emiss` represents a vector of numbers of categorical outcomes of each of dependent variables,  `n_dep` is the number of dependent variables, `group_labs` is a vector to be specified that includes dependent variables' names, `subj_nr` is an indicator of a subject for which we want to obtain plot and `mode` is an indicator of the way we wish to display it(`mode=1` by default indicating output grouped by states).
...
### Simulating data 
Knowing that our data follows a hidden Markov model (HMM) with a multilevel structure, we could also want to simulate data for multiple subjects.
In the multilevel structure each subject is allowed to have its own set of parameters. These parameters at the subject level(level 1) are tied together by normally distributed population distribution at level 2 for each of corresponding parameters. The package provides `sim_mHMM` function that allows a user to simulate data accounting for both heterogeneity between subjects that can be natural and/or unexplained and the fact that subjects parameters can depend on a covariate(s). 
Using the function we can simulate specific number of observations `n_t` for specified number of subjects `n`. Moreover, we can simulate
...
To simulate desirable data we specify the average population emission distribution(s) of the observations given the hidden states for each of the dependent variables(see examples below for different coding for categorical and continuous distributions) and average transition probability matrix. One could also specify `var_gamma` and/or `var_emiss` representing respectively variance between subjects in transition probability matrix and variance between subjects in the emission distribution(s). The default of both variables is set to $0.1$ corresponding yo little variation between subjects.
Here we can see an example of simulating data for $10$ subjects with each $100$ observations for `m=3` state hidden Markov Model (HMM). This is done for one dependent variable with `q_emiss=4` categorical outcomes with average emission probability distribution matrix `emiss_distr`. 
...
We could also want to add covariates matrices for simulating gamma and/or emission probability distribution matrices. We do that by simply including list of covariance matrices `xx_vec` with the first element on the list representing covariance matrix for transition probability matrix and rest representing covariance for each of dependent variables. When specifying covariates we also need to include a list of the regression parameters `beta` to predict gamma and/or emission distribution in combination with `xx_vec` using (multinomial logistic) regression. Here same as with `x_vec` the first element in the list is matrix used to predict the transition probability matrix gamma. The subsequent matrices are used to predict the emission distributions of the dependent variables. 
To simulate only selected objects we include covariates and beta coefficients in positions corresponding in `xx_vec` and `beta` lists. The rest of inputs that we are not interested in should be set to $0$. 
