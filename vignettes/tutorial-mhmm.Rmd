...
In the multilevel structure each subject is allowed to have its own set of parameters. These parameters at the subject level(level 1) are tied together by normally distributed population distribution at level 2 for each of corresponding parameters. The package provides `sim_mHMM` function that allows a user to simulate data accounting for both heterogeneity between subjects that can be natural and/or unexplained and the fact that subjects parameters can depend on a covariate. 
Using the function we can simulate either specific number of observations **n_t** for specified number of subjects **n** or simulate
the transition probability and/or emission distribution matrices for *n* number of subjects(we obtain the later simulation by setting *return_ind_par=TRUE*). The data can be simulated for **n_dep** dependent variables which distribution can be either 'categorical' or 'continuous'(we specify it in **data_distr** variable). 
...
We could also want to add covariates matrices for simulating gamma and/or emission probability distribution matrices. We do that by simply including list of covariance matrices *xx_vec* with the first element on the list representing covariance matrix for transition probability matrix and rest representing covariance for each of dependent variables. When specifying covariates we also need to include a list of the regression parameters to predict gamma and/or emission distribution in combination with xx_vec using (multinomial logistic) regression. Here same as with **x_vec** the first element in the list is matrix used to predict the transition probability matrix gamma. The subsequent matrices are used to predict the emission distributions of the dependent variables. To simulate only selected objects using given set of covariates and beta coefficients we can include them in *xx_vec* and *beta* lists positions corresponding to them and set objects the rest as NULL. For coding covariance for continuous data see example below or type '?sim_mHMM' in console. 
...
```
Comamnd *data2$subject_gamma* returns a list containing n elements with the simulated subject specific transition probability matrices gamma.


Lastly you would like to account for continuous distribution of multiple dependent variables :
