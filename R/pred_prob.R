...
#'   \code{\link{mHMM}}.
#' @param covariate A numeric vector specifying the values of covariates.
...
pred_probs <- function(object, covariate, component = "gamma", dep = 1, cat_lab, dep_lab, print.df = TRUE,...){
  ...
  if(component == "gamma"){
    ...
    for(c in 1:length(covariate)){
      ...
      cov_mat <- cbind(rep(0, m), matrix(apply(cov[((burn_in + 1): J),], 2, median), byrow = TRUE, ncol = q_emiss[dep]-1, nrow = m) * covariate[c])
