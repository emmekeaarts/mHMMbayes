...
#'
#' \code{mHMM_tv} fits a multilevel (also known as mixed or random effects)
#' hidden Markov model (HMM) with ONE time varying covariate to predict the
#' transition probabilities gamma. The model is designed to be used on intense
#' longitudinal data with categorical, continuous (i.e., normally distributed),
#' or count (i.e., Poisson distributed) observations of multiple subjects and is
#' fitted using Bayesian estimation. The function \code{mHMM_tv} creates an
#' object of class \code{mHMM_tv}. By using a multilevel framework, we allow for
...
#'
#' Person level covariates specified in \code{xx} and time-varying covariates
#' specified in \code{xx_t} can either be dichotomous or
#' continuous variables. Dichotomous variables have to be coded as 0/1
#' variables. Categorical or factor variables can as yet not be used as
#' predictor covariates. The user can however break up the categorical variable
#' in multiple dummy variables (i.e., dichotomous variables), which can be used
#' simultaneously in the analysis. Continuous predictors are automatically
#' centered. That is, the (grand) mean value of the covariate is subtracted from
#' all values of the covariate such that the new mean equals zero. This is done
#' such that the presented probabilities in the output (i.e., for the population
...
#'   covariates.
#' @param xx_t An optional matrix of time-varying (level 1) covariates to
#'   predict the transition probabilities gamma. In the matrix \code{xx_t}, the
#'   rows represent the observations over time and the first column indicates
#'   subject id number. Hence, the id number is repeated over rows equal to the
#'   number of observations for that subject. The subsequent columns contain the
#'   time-varying covariate(s) used to predict the transition probabilities
#'   gamma. The total number of rows are equal to the sum over the number of
#'   observations of each subject, and the number of columns are equal to the
#'   number of covariate(s) + 1.
...
mHMM_tv <- function(s_data, data_distr = 'categorical', gen, xx = NULL, xx_t = NULL, start_val, mcmc, return_path = FALSE, show_progress = TRUE,
                 gamma_hyp_prior = NULL, emiss_hyp_prior = NULL, gamma_sampler = NULL, emiss_sampler = NULL){
  # Note: in current version not possible to manually specify gamma_sampler and gamma_hyp_prior related to the time varying covariate,
  # for the conventional part of gamma still possible in this version.
  ...
  if(ncol(s_data) != n_dep + 1){
    stop(paste("The number of columns in s_data should equal ", n_dep + 2, ":", "\n", "first column denoting subject id, subsequently ", n_dep, " column(s) for the dependent variable(s).", sep = ""))
  }
  if(is.null(xx_t)){
    n_tv_cov    <- 0 # CHECK
  } else {
    n_tv_cov    <- ncol(xx_t) - 1 # CHECK: add check for missing?
    # Grand mean centering:
    for(j in 2:(n_tv_cov + 1)){
      if(is.factor(xx_t[,j])){
        stop("Factors currently cannot be used as time-varying covariates, see help file for alternatives")
      }
      if((length(unique(xx_t[,j])) == 2) & (sum(xx_t[,j] != 0 & xx_t[,j] !=1) > 0)){
        stop("Dichotomous time-varying covariates in xx need to be coded as 0 / 1 variables. That is, only conisting of the values 0 and 1")
      }
      if(length(unique(xx_t[,j])) > 2){
        xx_t[,j] <- xx_t[,j] - mean(xx_t[,j])
      }
    }
  }
  for(s in 1:n_subj){
  ...
  for(s in 1:n_subj){
    ...
    n_vary[s] <- n_t
    if(n_tv_cov == 0){
      subj_data[[s]]$xx_t <- matrix(1, nrow = n_vary[s], ncol = 1)
    } else {
      subj_data[[s]]$xx_t <- matrix(c(rep(1,n_vary[s]),xx_t[xx_t[,1] == id[s],2:(ncol(xx_t))]), ncol = (n_tv_cov + 1), byrow = FALSE)
    }
    subj_data[[s]]	<- c(subj_data[[s]], n_t = n_t, list(gamma_converge = numeric(m),
                                                        gamma_int_mle = matrix(NA_real_, m, (m - 1) * (1 + n_tv_cov)), # CHECK
                                                        gamma_mhess = matrix(NA_real_, (m - 1) * (1 + n_tv_cov), (m - 1) * (1 + n_tv_cov)), # CHECK
                                                        emiss_mle = emiss_mle,
  ...
  if(m > 1){
    if(is.null(gamma_sampler)) {
      ...
      gamma_bet_mle0  <- rep(0, (n_tv_cov)*(m-1)) # CHECK dimensions: (m-1) or (n_tv_cov)*(m-1)?
      gamma_scalar    <- 2.93 / sqrt(m - 1)
      gamma_bet_scalar    <- 2.93 / sqrt(n_tv_cov * (m - 1)) # CHECK 2: multiply denominator by n_p?
      gamma_w         <- .1
      gamma_bet_w         <- .1
    } else {
  ...
  if(m > 1){
    if(is.null(gamma_hyp_prior)){
      ...
      gamma_mu0_bet   <- rep(list(matrix(0,nrow = n_tv_cov, ncol = m - 1)), m) # CHECK 3.a: nrows = number of tv covariates or number of tv covariates + 1?
      # gamma_V0_bet    <- (3 + m - 1) * diag(m - 1)
      gamma_V0_bet    <- (3 + m - 1) * diag(n_tv_cov * (m - 1)) # CHECK 3.b: nrows = number of tv covariates?
    } else {
  ...
  if(m > 1){
    ...
    gamma_c_bet           <- rep(list(matrix(0, nrow = n_tv_cov, ncol = m - 1)), m) # CHECK 4: nrow = n_tv_cov or (n_tv_cov + 1) dimensions?
    gamma_mu_int_bar      <- gamma_V_int <- vector("list", m)
    gamma_int_naccept         <- matrix(0, n_subj, m)
    gamma_bet_naccept     <- matrix(0, 1, m) # CHECK 5: nrows = n_p? or single row?
  ...
  if(m > 1){
    ...
    # gamma_tv_cov            <- matrix(NA_real_, ncol = (m-1)*m, nrow = J)
    gamma_tv_cov            <- matrix(NA_real_, nrow = J, ncol = ((m-1) * m) * (n_tv_cov)) # CHECK: ncol = (m-1)*m*n_p?
    # colnames(gamma_tv_cov)  <- paste("tv_cov","_S", rep(1:m, each = (m-1)), "toS", rep(2:m, m), sep = "") # CHECK: tv covariate names?
    colnames(gamma_tv_cov)  <- paste( paste("cov", rep(1 : (n_tv_cov), each = (m-1)), "_", sep = ""), "S", rep(1:m, each = (m-1) * (n_tv_cov)), "toS", rep(2:m, m * (n_tv_cov)), sep = "") # CHECK: tv covariate names?

    # Values for first iteration of gamma_tv_cov
    gamma_tv_cov[1,] <- unlist(lapply(gamma_mu0_bet, "[",))
  ...
  for (iter in 2 : J){
    ...
    if(m > 1){
      ...
      for(s in 1:n_subj){
        ...
        gamma_t       <- timevar_gamma(int = matrix(gamma_int_subj[[s]][iter-1,], byrow = TRUE, ncol = m-1),
                                       ...
                                       bet = gamma_c_bet, # CHECK: check gamma_c_bet is list of M elements, each a matrix of Np by (M-1)
        ...
        if(data_distr == 'categorical'){
          ...
          forward				<- cont_tv_mult_fw_r_to_cpp(x = subj_data[[s]]$y, m = m, emiss = emiss[[s]], tgamma = gamma_t, n_dep = n_dep, delta=NULL)
        } else if(data_distr == 'count'){
          forward				<- count_tv_mult_fw_r_to_cpp(x = subj_data[[s]]$y, m = m, emiss = emiss[[s]], tgamma = gamma_t, n_dep = n_dep, delta=NULL)
        }
        alpha         <- abs(forward[[1]])
        ...
        for (i in 1:m){
          if(!is.null(trans[[s]][[i]])){
            ...
            # xx_t_state[[s]][[i]] <- subj_data[[s]]$xx_t[-n_vary[s],][which(sample_path[[s]][-n_vary[s],iter] == i),-1] # CHECK
            xx_t_state[[s]][[i]] <- matrix(subj_data[[s]]$xx_t[-n_vary[s],][which(sample_path[[s]][-n_vary[s],iter] == i),-1], ncol = n_tv_cov) # CHECK: format as matrix (or column vector if ncol = 1?)
          }
          for(q in 1:n_dep){
    ...
    for(i in 1:m){
      ...
      if(m > 1){
        ...
        xx_t_p              <- matrix(c(rep(1, sum(sapply(xx_t_state, lengths)[i,]/n_tv_cov)),
                                        do.call(rbind, lapply(xx_t_state, "[[", i))), ncol = 1 + n_tv_cov) # CHECK: ncol = Np or (Np + 1)?

        # X                   <- makeX(xx_t = matrix(c(rep(1, length(xx_t_p[,2]) + m), c(xx_t_p[,2], rep(0,m))), ncol = 2), n_cat = m)
        X                   <- makeX(xx_t = matrix(c(rep(1, length(xx_t_p[,-1])/n_tv_cov + m),
                                                     rbind(matrix(xx_t_p[,-1], ncol = n_tv_cov),
                                                           matrix(rep(0,m), nrow = m, ncol = n_tv_cov) ) ),
                                                   ncol = 1 + n_tv_cov),
                                     n_cat = m) # CHECK
        gamma_mle_pooled		<- optim(c(gamma_int_mle0[i,], gamma_bet_mle0), llmnl_bet_optim, Obs = c(Obs_p[,2], 1:m),
                                   X = X, n_cat = m, method = "BFGS", hessian = FALSE,
                                   control = list(fnscale = -1)) # CHECK shape of gamma_bet_mle0
        ...
        gamma_hess_mle_pooled[[i]] <- mnlHess_bet(bet = gamma_mle_pooled$par, n_cat = m,
                                                    ...
                                                    # xx_t = rbind(xx_t_p, matrix(rep(c(0,1), each = m), ncol = 1 + n_tv_cov))) # CHECK ncol
                                                    xx_t = rbind(xx_t_p, matrix(c(rep(0,m),rep(1, each = m * n_tv_cov)), ncol = 1 + n_tv_cov)))
      ...
      for (s in 1:n_subj){
        ...
        if (m > 1){
          ...
          if(gamma_out$convergence == 0){
            ...
            subj_data[[s]]$gamma_mhess	<- mnlHess_bet(bet = gamma_out$par,
                                                      n_cat = m,
                                                      Obs = c(trans[[s]][[i]], c(1:m)),
                                                      # xx_t = matrix(c(rep(1, length(xx_t_state[[s]][[i]]) + m),
                                                      #                 c(xx_t_state[[s]][[i]], rep(0,m))), ncol = 2))
                                                      xx_t = matrix(c(rep(1, length(xx_t_state[[s]][[i]])/n_tv_cov + m),
                                                                      rbind(xx_t_state[[s]][[i]], matrix(rep(0,m), nrow = m, ncol = n_tv_cov) )), ncol = 1 + n_tv_cov))
          } else {
            subj_data[[s]]$gamma_converge[i] <- 0
            subj_data[[s]]$gamma_int_mle[i,] <- rep(0, (m - 1) * (1 + n_tv_cov))
            subj_data[[s]]$gamma_mhess	<- diag((m-1) * (1 + n_tv_cov))
          }
          # if this is first iteration, use MLE for current values RW metropolis sampler
          if (iter == 2){
            gamma_c_int[[i]][s,]		<- gamma_out$par[1:(m-1)] # CHECK: this should be correct
          }
        }
      ...
      if(m > 1){
        for (s in 1:n_subj){
          ...
          gamma_int_candcov_comb 			<- chol2inv(chol(subj_data[[s]]$gamma_mhess[1:(m-1), 1:(m-1)] +
                                                       chol2inv(chol(gamma_V_int[[i]])))) # CHECK: Redefine Hessian?
          gamma_int_RWout					    <- mnl_RW_once_int_fixed_bet(int1 = matrix(gamma_c_int[[i]][s,], nrow = 1),
                                                               Obs = c(trans[[s]][[i]], 1:m),
                                                               n_cat = m,
                                                               # xx_t =  matrix(c(rep(1,length(xx_t_state[[s]][[i]]) + m), xx_t_state[[s]][[i]], rep(0,m)), ncol = 2),
                                                               xx_t =  matrix(c(rep(1,length(xx_t_state[[s]][[i]])/n_tv_cov + m),
                                                                                rbind(xx_t_state[[s]][[i]], matrix(rep(0,m), nrow = m, ncol = n_tv_cov ) ) ),
                                                                              ncol = 1 + n_tv_cov), # CHECK
                                                               # bet = gamma_c_bet[[i]],
                                                               bet = gamma_c_bet[[i]], # CHECK what to do here?
                                                               mu_int_bar1 = c(t(gamma_mu_int_bar[[i]]) %*% xx[[1]][s,]),
                                                               V_int1 = gamma_V_int[[i]],
                                                               scalar = gamma_scalar,
                                                               candcov1 = gamma_int_candcov_comb) # CHECK dimensions of xx_t_state
          gamma_c_int[[i]][s,]		 <- gamma_int_RWout$draw_int
          gamma[[s]][i,]  	      <- PD_subj[[s]]$trans_prob[iter, ((i-1) * m + 1) : ((i-1) * m + m)] <- (gamma_int_RWout$prob + .0001) / (1 + 0.0001 *m)
        ...
        gamma_bet_candcov_comb 			<- chol2inv(chol(gamma_hess_mle_pooled[[i]][m:((1+n_tv_cov)*(m-1)), m:((1+n_tv_cov)*(m-1))] +
                                                     chol2inv(chol(gamma_V0_bet)))) # CHECK dimensions of Hessian
        gamma_bet_RWout					    <- mnl_RW_once_fixed_bet_pooled(bet1 = gamma_c_bet[[i]], Obs_p = Obs_p, n_cat = m,
                                                                ...
                                                                mu_bet_bar1 = gamma_mu0_bet[[i]],
                                                                V_bet1 = gamma_V0_bet,
                                                                scalar = gamma_bet_scalar,
                                                                candcov2 = gamma_bet_candcov_comb,
                                                                n_p = n_tv_cov)
    ...
    if(m > 1){
      ...
      if( (n_tv_cov + 1) > 1){
        gamma_tv_cov[iter,]      	<- unlist(lapply(gamma_c_bet, "[",)) # CHECK
        # gamma_tv_cov[iter,]      	<- unlist(lapply(gamma_c_bet, "[",-1,)) # CHECK
      }
    } else if(m == 1){
  ...
  if(data_distr == 'categorical'){
    if(return_path == TRUE){
      out <- list(input = list(data_distr = data_distr, m = m, n_dep = n_dep, q_emiss = q_emiss, J = J,
                  ...
                  emiss_prob_bar = emiss_prob_bar, gamma_int_naccept = gamma_int_naccept, gamma_bet_naccept = gamma_bet_naccept,
    ...
    class(out) <- append(class(out), c("mHMM_tv", "cat"))
  } else if(data_distr == 'continuous'){
    if(return_path == TRUE){
      out <- list(input = list(data_distr = data_distr, m = m, n_dep = n_dep, J = J,
                  ...
                  gamma_tv_cov = gamma_tv_cov,
                  gamma_V_int_bar = gamma_V_int_bar,
                  emiss_cov_bar = emiss_cov_bar, gamma_prob_bar = gamma_prob_bar,
                  emiss_mu_bar = emiss_mu_bar, gamma_int_naccept = gamma_int_naccept,
    ...
    class(out) <- append(class(out), c("mHMM_tv", "cont"))
  } else if(data_distr == 'count'){
    if(return_path == TRUE){
      out <- list(input = list(data_distr = data_distr, m = m, n_dep = n_dep, J = J,
                               burn_in = burn_in, n_subj = n_subj, n_vary = n_vary, dep_labels = dep_labels),
                  ...
                  gamma_int_naccept = gamma_int_naccept,
                  gamma_bet_naccept = gamma_bet_naccept,
                  emiss_naccept = emiss_naccept,
                  sample_path = sample_path)
    } else {
    ...
    class(out) <- append(class(out), c("mHMM_tv","count"))
  }
  return(out)
}
