#' @export
#'
summary.mHMM_f <- function(object, ...){
  ...
  for(i in 1:n_dep){
    EM_pop[[i]] <- matrix(round(c(apply(matrix(object$emiss_mu[[i]][((burn_in + 1): J),], ncol = m), 2, median),
                                   apply(matrix(object$emiss_sd[[i]][((burn_in + 1): J),], ncol = m), 2, median)),
                                3), ncol = 2, nrow = m)
