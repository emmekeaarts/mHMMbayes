...
#'
#' \code{sim_mHMM_tv} simulates data for multiple subjects, for which the data
#' have either categorical, continuous (i.e., normally distributed), or count
#' (i.e., Poisson distributed) observations that follow a hidden Markov model
#' (HMM) with a multilevel structure. In addition, within \code{sim_mHMM_tv} the
#' option of specifying time-varying covariate(s) that predict the transition
#' probabilities gamma over time is incorporated. The multilevel structure
#' implies that each subject is allowed to have its own set of parameters, and
#' that the parameters at the subject level (level 1) are tied together by a
#' population distribution at level 2 for each of the corresponding parameters.
#' The shape of the population distribution for each of the parameters is a
#' normal distribution. In addition to (natural and/or unexplained)
#' heterogeneity between subjects, the subjects parameters can also depend on a
#' subject level, fixed over time, covariate(s).
#'
...
sim_mHMM_tv <- function(n_t, n, data_distr = 'categorical', gen, gamma, emiss_distr, start_state = NULL,
                     xx_vec = NULL, beta = NULL, xx_t = NULL, beta_t = NULL, var_gamma = 0.1, var_emiss = NULL,
  ...
  if(is.null(xx_vec)){
    ...
    xx_vec[[1]] <- matrix(rep(1,n), nrow = n, ncol = 1)
    for(i in 2:(n_dep + 1)){
      xx_vec[[i]] <- matrix(rep(1,n), nrow = n, ncol = 1)
    }
  } else {
    if(is.null(xx_vec[[1]])) {
          xx_vec[[1]] <- matrix(rep(1,n), nrow = n, ncol = 1)
  ...
  if(is.null(beta)){
    beta <- rep(list(NULL), 2)
    beta[[1]] <- rep(list(matrix(0, ncol = m - 1, nrow = 1)), m)
    beta[[2]] <-  vector("list", n_dep)
    if(data_distr == 'categorical'){
      for(q in 1:n_dep){
        beta[[2]][[q]] <- rep(list(matrix(0, nrow = 1, ncol = q_emiss[q]-1)), m)
        }
      } else if (data_distr == 'continuous'){
        for(q in 1:n_dep){
          beta[[2]][[q]] <- matrix(0, nrow = 1, ncol = m)
          }
        } else if (data_distr == 'count'){
          for(q in 1:n_dep){
            beta[[2]][[q]] <- matrix(0, nrow = 1, ncol = m)
      ...
      if(is.null(beta[[1]])) {
        beta[[1]] <- rep(list(matrix(0, ncol = m - 1, nrow = ncol(xx_vec[[1]]))), m)
        }
      if(all(sapply(beta[[2]], is.null))){
        beta[[2]] <-  vector("list", n_dep)
             if(data_distr == 'categorical'){
               for(q in 1:n_dep){
                 beta[[2]][[q]] <- rep(list(matrix(0, nrow = ncol(xx_vec[[2]]), ncol = q_emiss[q]-1)), m)
               }
             } else if (data_distr == 'continuous'){
               for(q in 1:n_dep){
                 beta[[2]][[q]] <- matrix(0, nrow = ncol(xx_vec[[2]]), ncol = m)
               }
  ...
  if(m > 1){
    ...
    sub_gamma_int <- rep(list(NULL), n)
    sub_gamma_t <- rep(list(NULL), n)
  ...
  for(j in 1:n){
    if(m > 1){
      sub_xb <- matrix(NA_real_, nrow = m, ncol = m-1)
      for(i in 1:m){
        sub_xb[i,] <- xx_vec[[1]][j,] %*% beta[[1]][[i]]
      }
      sub_xx_t <- matrix(xx_t[xx_t[,1] == j,-1])
      sub_xbt <- do.call(cbind, lapply(1:m, function(i,j) sub_xx_t %*% beta_t[[i]], j)) # CHECK
      sub_gamma_zero <- matrix(as.numeric(t(mnl_gamma + sub_xb + rnorm(n = m * (m-1), mean = 0, sd = sqrt(as.numeric(var_gamma))))),
                               nrow = n_t, ncol = m*(m-1), byrow = TRUE)
      sub_gamma[[j]] <- int_to_prob(matrix(sub_gamma_zero[1,], nrow = m, ncol = m-1, byrow = TRUE))
      sub_gamma_int[[j]] <- sub_gamma_zero + sub_xbt
      sub_gamma_t[[j]] <- do.call(rbind, lapply(1:nrow(sub_gamma_int[[j]]),
                                                function(t) matrix(as.numeric(t(int_to_prob(matrix(sub_gamma_int[[j]][t,], nrow = m, ncol = m-1, byrow = TRUE)))), nrow = 1, ncol = m*m) ) )
    }
    for(i in 1:n_dep){
      if(data_distr == "categorical"){
        sub_xb <- matrix(NA_real_, nrow = m, ncol = q_emiss[i]-1)
        for(h in 1:m){
          sub_xb[h,] <- xx_vec[[2]][j,] %*% beta[[2]][[i]][[h]]
        }
        sub_emiss[[j]][[i]] <- int_to_prob(mnl_emiss[[i]] + sub_xb +
                                             rnorm(n = m * (q_emiss[i]-1), mean = 0, sd = sqrt(as.numeric(var_emiss[[i]]))))
      } else if(data_distr == "continuous"){
        sub_emiss[[j]][[i]] <- emiss_distr[[i]]
        sub_emiss[[j]][[i]][,1] <- emiss_distr[[i]][,1] +  xx_vec[[1+i]][j,] %*% beta[[2]][[i]] +
          rnorm(n = m, mean = 0, sd = sqrt(as.numeric(var_emiss[[i]])))
      } else if(data_distr == "count"){
        sub_emiss[[j]][[i]] <- emiss_distr[[i]]
        sub_emiss[[j]][[i]][,1] <- exp(emiss_distr[[i]][,1] +  xx_vec[[1+i]][j,] %*% beta[[2]][[i]] +
                                         rnorm(n = m, mean = 0, sd = sqrt(as.numeric(var_emiss[[i]]))))
      }
    }

    if(n_t != 0){
      init <- solve(t(diag(m) - int_to_prob(matrix(sub_gamma_int[[j]][1,], nrow = m, ncol = m-1, byrow = TRUE)) + 1), rep(1, m))
      ...
      for(t in 2:n_t){
        # states[((j-1) * n_t + t), 2] <- sample(x = 1:m, size = 1, prob = int_to_prob(matrix(sub_gamma_int[[j]][t,], nrow = m, ncol = m-1, byrow = TRUE))[states[((j-1) * n_t + t - 1), 2],])
        states[((j-1) * n_t + t), 2] <- sample(x = 1:m, size = 1, prob = int_to_prob(matrix(sub_gamma_int[[j]][t-1,], nrow = m, ncol = m-1, byrow = TRUE))[states[((j-1) * n_t + t - 1), 2],])
        if(data_distr == "categorical"){
          for(i in 1:n_dep){
  ...
  if (return_ind_par == FALSE & n_t != 0){
    ...
    return(list(states = states, obs = obs, subject_gamma = sub_gamma, subject_emiss = sub_emiss, subject_gamma_t = sub_gamma_t))
  } else if (n_t == 0){
