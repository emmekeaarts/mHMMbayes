...
#'
#' \code{sim_mHMM_v} simulates data for multiple subjects, for which the data have
#' continuous (i.e., normally distributed)
...
#'   \code{emiss_distr} of the dependent variable(s). For \code{gamma}, one
#'   regression parameter is specified for each element in \code{gamma}, with
#'   the following exception. The first element in each row of \code{gamma} is
#'   used as reference category in the Multinomial logistic regression. As such,
#'   no regression parameters can be specified for these parameters. Hence, the
...
#'   equal to \code{m} - 1.  See \emph{details} for more information. For
#'   continuous and count emission distributions, the subsequent elements in the
#'   list \code{beta} consist of a matrix with the number of rows equal to
#'   \code{m} and 1 column.
#'
...
#' @param var_emiss Either a numeric vector with length \code{n_dep} or a list
#'   of \code{n_dep} matrices denoting the amount of variance between subjects
#'   in the emission distribution(s). When opting for a list of matrices: each
#'   element is a matrix with \code{m} rows and 1 column; the single column
#'   denoting the variance between the subjects' means of state \emph{i} (row
#'   \emph{i}) of the normal distribution used as prior for the Normal
#'   emissions. This value corresponds to the variance in the mean of the
#'   emission distribution(s) across subjects. Note that if only one variance
#'   value provided for each emission distribution, the variance is assumed
#'   fixed across states within  an emission distribution. The default equals
#'   0.1. Note that if data for only 1 subject is simulated (i.e., n = 1),
#'   \code{var_emiss} is set to a vector of 0's.
...
#'
#' data_cont <- sim_mHMM_v(n_t = n_t, n = n, data_distr = 'continuous',
...
sim_mHMM_v <- function(n_t, n, data_distr = 'categorical', gen, gamma, emiss_distr, start_state = NULL,
                     xx_vec = NULL, beta = NULL, var_gamma = 0.1, var_emiss = NULL,
                     return_ind_par = FALSE, m, n_dep){
