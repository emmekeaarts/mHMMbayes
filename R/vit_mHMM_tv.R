...
#' algorithm
#'
#' \code{vit_mHMM_tv} obtains the most likely state sequence (for each subject)
#' from an object of class \code{mHMM_tv} (generated by the function
#' \code{mHMM_tv()}), using (an extended version of) the Viterbi algorithm. This is
...
#' output object \code{return_path} of the function \code{mHMM_tv()}.
#'
#' @param object An object of class \code{mHMM_tv}, generated
#'   by the function \code{\link{mHMM_tv}}.
#' @inheritParams mHMM
...
#'
#' @return The function \code{vit_mHMM_tv} returns a matrix containing the most
...
#' ## OLD EXAMPLES, THESE ARE FOR vit_mHMM()
...
#'
#'
#' @seealso \code{\link{mHMM_tv}} for analyzing multilevel hidden Markov data
...
#'
#'
vit_mHMM_tv <- function(object, s_data, xx_t, burn_in = NULL, return_state_prob = FALSE){
  ...
  if (!is.mHMM_tv(object)){
    stop("The input object used should be from the class mHMM_tv, obtained by using the function mHMM_tv.")
  }
  ...
  xx_t_c <- xx_t
  xx_t_c[,2] <- xx_t_c[,2] - mean(xx_t_c[,2])
  n_tv_cov <- ncol(xx_t_c) - 1
  gamma_bet <- apply(object$gamma_tv_cov[((burn_in + 1): J),], 2, median)
  gamma_bet2 <- vector("list", m)
  for(i in 1:m){
    gamma_bet2[[i]] <- matrix(gamma_bet[(((i-1) * (m-1)) + 1):(i * (m-1))], nrow = n_tv_cov)
  }
  for(s in 1:n_subj){
    emiss   <- est_emiss[[s]]
    xx_t_c_s <- matrix(c(rep(1,n_vary[s]),xx_t_c[xx_t_c[,1] == id[s],2:(ncol(xx_t_c))]), ncol = (n_tv_cov + 1), byrow = FALSE)
    gamma_int_s <- matrix(apply(object$gamma_int_subj[[s]][((burn_in + 1): J),], 2, median), byrow = TRUE, ncol = m-1)
    gamma_t       <- timevar_gamma(int = gamma_int_s,
                                   bet = gamma_bet2, # CHECK: check gamma_c_bet is list of M elements, each a matrix of Np by (M-1)
                                   xx_t = xx_t_c_s, m = m)
    if(data_distr == 'categorical'){
      probs[[s]]				<- cat_tv_mult_fw_r_to_cpp(x = as.matrix(s_data[s_data[,1] == id[s],][,-1], ncol = n_dep),
                                               m = m, emiss = emiss, tgamma = gamma_t, n_dep = n_dep, delta=NULL)[[1]]
    } else if(data_distr == 'continuous'){
      probs[[s]]				<- cont_tv_mult_fw_r_to_cpp(x = as.matrix(s_data[s_data[,1] == id[s],][,-1], ncol = n_dep),
                                                m = m, emiss = emiss, tgamma = gamma_t, n_dep = n_dep, delta=NULL)[[1]]
    } else if(data_distr == 'count'){
      probs[[s]]				<- count_tv_mult_fw_r_to_cpp(x = as.matrix(s_data[s_data[,1] == id[s],][,-1], ncol = n_dep),
                                                 m = m, emiss = emiss, tgamma = gamma_t, n_dep = n_dep, delta=NULL)[[1]]
    }
