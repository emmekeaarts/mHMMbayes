...
summary.mHMM <- function(object, ...){
  ...
  if (!is.null(type_covar[[1]])){
    regr_coeff_gamma <- data.frame(From_state = c(rep(1:m, each = m - 1)),
                                   To_state = rep(2:m, m)) %>%
      dplyr::mutate(CrI_lower = round(apply(object$gamma_cov_bar[(burn_in + 1): J,], 2, quantile, probs = 0.025),3),
             CrI_upper = round(apply(object$gamma_cov_bar[(burn_in + 1): J,], 2, quantile, probs = 0.975),3),
             Beta = ifelse(CrI_lower < 0 & CrI_upper > 0, round(apply(object$gamma_cov_bar[(burn_in + 1): J,], 2, mean),3), paste0(round(apply(object$gamma_cov_bar[(burn_in + 1): J,], 2, mean),3), "*"))
  ...
  if (!is.null(type_covar[[2]])){
    ...
    for (i in 1:n_dep){
      regr_coeff_emiss[[i]] <- data.frame(Category = c(rep(2:q_emiss[i], m)),
        ...
        dplyr::mutate(CrI_lower = round(apply(object$emiss_cov_bar[[i]][(burn_in + 1): J,], 2, quantile, probs = 0.025),3),
               CrI_upper = round(apply(object$emiss_cov_bar[[i]][(burn_in + 1): J,], 2, quantile, probs = 0.975),3),
               Beta = ifelse(CrI_lower < 0 & CrI_upper > 0, round(apply(object$emiss_cov_bar[[i]][(burn_in + 1): J,], 2, mean),3), paste0(round(apply(object$emiss_cov_bar[[i]][(burn_in + 1): J,], 2, mean),3), "*"))
        ) %>%
