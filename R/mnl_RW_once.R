...
mnl_RW_once <- function(int1, Obs, n_cat, mu_int_bar1, V_int1, scalar, candcov1) {
  ...
  oldpostlike	 		<- oldloglike + mvtnorm::dmvnorm(int1, mu_int_bar1, V_int1, log = TRUE)
  ...
  newpostlike	 		<- newloglike + mvtnorm::dmvnorm(int_new, mu_int_bar1, V_int1, log = TRUE)
...
mnl_RW_once_fixed_bet_pooled <- function(bet1, Obs_p, xx_t_p, n_cat, mu_bet_bar1, int_p, V_bet1, scalar, candcov2, n_subj, n_p) {
  # obtain likelihood and transition prob with the parameters sampled in the previous iteration and current sampled state sequence
  bet_p           <- cbind(int_p, matrix(bet1, byrow = TRUE, ncol = n_p * (n_cat - 1), nrow = n_subj))
  oldloglike	 		<- llmnl_bet_pooled(bet_p = bet_p, Obs_p = Obs_p, xx_t_p = xx_t_p, n_cat = n_cat, n_subj = n_subj)
  oldpostlike	 		<- oldloglike + dmvnorm(as.numeric(bet1), mu_bet_bar1, V_bet1, log = TRUE)

  # obtain new parameters for gamma from proposal distribution plus new likelihood
  bet_new		 		  <- as.numeric(bet1) + rmvnorm(1, rep(0, n_p * (n_cat - 1)), scalar^2 * candcov2, method = "svd")
  bet_p_new       <- cbind(int_p, matrix(bet_new, byrow = TRUE, ncol = n_p * (n_cat - 1), nrow = n_subj))
