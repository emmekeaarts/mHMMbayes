...
plot.mHMM <- function(x, component = "gamma", dep = 1, col, cat_lab,
  ...
  if(nzchar(system.file(package = "ggplot2")) && nzchar(system.file(package = "dplyr"))){
    # group level gamma
    gg <- as.data.frame(object$gamma_prob_bar) %>% mutate(id = "group")
    # subject level gamma
    sg <- object$PD_subj %>% purrr::map(
      ~ as.data.frame(.x) %>% dplyr::select(tidyselect::starts_with("S"))
    ...
    rbind(sg, gg) %>%
      ...
      # create plots
      ggplot2::ggplot(ggplot2::aes(x = value, color = to_state, alpha = id, linetype = lty, size = lty)) +
      ggplot2::geom_density() +
      ggplot2::scale_size_manual(values = c("1" = 0.7, "2" = 0.3))+
