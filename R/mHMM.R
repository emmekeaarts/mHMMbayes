...
mHMM <- function(s_data, gen, xx = NULL, start_val, mcmc, return_path = FALSE, print_iter, show_progress = TRUE,
  ...
  for (iter in 2 : J){
    ...
    for(i in 1:m){
      ...
      ind <- which(apply(optim_pooled_gamma[[i]][,1:m], 1, function(x, want)
        isTRUE(all.equal(as.vector(x), as.vector(want))), table(trans_pooled)))
      if(length(ind) != 0){
      ...
      for(q in 1:n_dep){
        ...
        ind <- which(apply(optim_pooled_emiss[[i]][[q]][,1:q_emiss[q]], 1, function(x, want)
          isTRUE(all.equal(as.vector(x), as.vector(want))), table(cond_y_pooled)))
        if(length(ind) != 0){
      ...
      for (s in 1:n_subj){
        ...
        ind <- which(apply(optim_subj_gamma[[i]][,1:m], 1, function(x, want)
          isTRUE(all.equal(as.vector(x), as.vector(want))), table(c(trans[[s]][[i]], c(1:m)))))
        if(length(ind) != 0){
        ...
        for(q in 1:n_dep){
          ind <- which(apply(optim_subj_emiss[[i]][[q]][,1:q_emiss[q]], 1, function(x, want)
            isTRUE(all.equal(as.vector(x), as.vector(want))), table(c(cond_y[[s]][[i]][[q]], c(1:q_emiss[q])))))
          if(length(ind) != 0){
