...
#'   show_dprogress instead to show the progress of the algorithm.
...
#'   containing user specified parameters of the hyper-prior distribution on the
#'   transition probability matrix gamma, generated by the function
...
mHMM <- function(s_data, gen, xx = NULL, start_val, mcmc, return_path = FALSE, print_iter, show_progress = TRUE,
  ...
  if(is.null(gamma_hyp_prior)){
    ...
    if (!is.mHMM_prior_gamma(object)){
      stop("The input object specified for gamma_hyp_prior should be from the class mHMM_hyp_prior, obtained by using the function prior_gamma.")
    }
    if (gamma_hyp_prior$m != m){
      stop("The number of states specified in m is not equal to the number of states specified when creating the informative hper-prior distribution on gamma using the function prior_gamma.")
    }
    if (gamma_hyp_prior$n_xx != nx[1]){
      ...
      for (s in 1:n_subj){
        ...
        for(q in 1:n_dep){
          emiss_out				<- optim(emiss_int_mle_pooled[[i]][[q]], llmnl_int_frac, Obs = c(cond_y[[s]][[i]][[q]], c(1:q_emiss[q])),
                                  ...
                                  w = emiss_w, wgt = wgt, method = "BFGS", hessian = FALSE, control = list(fnscale = -1))
