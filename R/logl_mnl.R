...
## xx_t is matrix with observed time varying covariates, with first colum being a vector of 1's denoting the intercept
## note that for xx_t, we only use the observed values for time points at the sampled state sequence where state == i.
...
## and in the remaining n_cat - 1 columns (currently we only support incorporating ONE time varying covariate)
## is the current estimate of the beta's which are fixed over the subjects
## Obs2 is a matrix, with subject indicator in the first column, and the observed outcomes in the second column
llmnl_bet_pooled <- function(bet_p, Obs_p, xx_t_p, n_cat, n_subj){
  ...
  Xbeta <- NULL
  for(i in 1:(n_subj)){
    X <- matrix(xx_t_p[Obs_p[,1] == i,], ncol = ncol(xx_t_p)) %x% Imod
