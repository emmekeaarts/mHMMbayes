...
#inside function checking if the list of class of x is nested (subject specific) or not(group specific)
  ...
  #define parameters n_dep and q_emiss
  n_dep<-length(x)
  q_emiss<-c()
  ...
  if (isNested(x)==T){
    ...
    if (is.null(subj_nr)){
      ...
      new<-vector(mode = "list", length = n_dep) #list to contain the emission probability matrices for each state
      for(n in 1:n_dep){
        new[[n]]<-x[[n]][[subj_nr]]
      }
...
#----- Colour setting
  ...
  for(s in 1:m){
    ...
    emission_states[[s]]<-as.data.frame(data) ##check
    data<-matrix(nrow=n_dep,ncol=max(q_emiss))
  }
  a<-emission_states
  ...
  for(p in 1:n_dep){
    plot(0,type='n',axes=FALSE,ann=FALSE)
    if(is.null(subj_nr)==F){
      legend("center",legend=colnames(new[[p]]),fill=coul,title=names(x)[p],cex =1,bty="n")
    }else{
      legend("center",legend=colnames(new[[p]]),fill=coul,title=names(x[p]),cex =1,bty="n")
    }
  }
  ...
  if(n_dep>3){
    ...
    for(p in 1:n_dep){
      ...
      if(is.null(subj_nr)==F){
        ...
        legend("center",legend=colnames(x[[p]]),fill=coul,title=names(x[p]),cex =1,bty="n")
      }
    }
    if(n_dep>3){
     empty<-((ceiling(n_dep/4)*4)-n_dep)
